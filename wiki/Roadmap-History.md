...
>
> This page preserves the legacy XCSV Roadmap status/history material that existed before XCSV-REFACTOR-DOCS-001. It contains old `SYNCED`, `PASS`, `PARTIAL`, build, PID, release and execution-receipt language. Treat every dated/runtime/status claim as evidence at the time it was written, not as current truth. Use [Roadmap](Roadmap) for durable planning direction, GitHub issues for live execution state, [System Components](System-Components) for source inventory, and `registry/current-state.json` for per-source observations.
