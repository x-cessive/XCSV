...
heading: Roadmap History
blurb: Historical roadmap/status evidence retained with currentness warnings.
order: 14
source: Roadmap-History.md
generated: true
source_authority: wiki/Roadmap-History.md
---

<!-- GENERATED FROM wiki/Roadmap-History.md BY tools/build-docs.ps1. DO NOT EDIT docs/wiki BY HAND. -->

> **HISTORICAL EVIDENCE**
>
> **NOT CURRENT EXECUTION AUTHORITY**
>
> **DATED CLAIMS REQUIRE REVERIFICATION**
>
> This page preserves the legacy XCSV Roadmap status/history material that existed before XCSV-REFACTOR-DOCS-001. It contains old `SYNCED`, `PASS`, `PARTIAL`, build, PID, release and execution-receipt language. Treat every dated/runtime/status claim as evidence at the time it was written, not as current truth. Use [Roadmap](Roadmap.html) for durable planning direction, GitHub issues for live execution state, [System Components](System-Components.html) for source inventory, and `registry/current-state.json` for per-source observations.
...
- PRISON-GRAYBOX-001 has Architect priority override and is
  ...
  basic graybox placement and selected-object baselining. Exile `5e86b4d` also
  makes the helpers mission-local so Eden can run
  `execVM "tools\eden\prison_site_candidates.sqf";` from the loaded mission. No
  `mission.sqm`/live PBO mutation occurred and no graybox pass is claimed before
  Architect selects the site. Site `A_NORTH_CENTRAL_COAST` (`[7140,11800,0]`,
  520m x 360m, heading 35) was selected 2026-08-12; the parametrized builder
  `tools/eden/prison_graybox_build.sqf` places the perimeter/interior as one
  undoable history step with classnames verified against the server classdump,
  still without saving `mission.sqm`.
